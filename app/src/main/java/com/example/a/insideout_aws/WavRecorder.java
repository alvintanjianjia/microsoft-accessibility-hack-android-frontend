...
public class WavRecorder {
    ...
    public static WavRecorder getInstanse() {
        WavRecorder result = null;
        int i=0;
        do {
            result = new WavRecorder(AudioSource.MIC,
                    sampleRates[i],
                    AudioFormat.CHANNEL_IN_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } while((++i<sampleRates.length) & !(result.getState() == WavRecorder.State.INITIALIZING));
        return result;
    }
    ...
    /**
     *
     * Returns the state of the recorder in a WavRecorder.State typed object.
...
//			Log.d(WavRecorder.this.getClass().getName(), state + ":" + numOfBytes);
            try {
                randomAccessWriter.write(buffer); 		  // write audio data to file
                payloadSize += buffer.length;
            } catch (IOException e) {
                Log.e(WavRecorder.class.getName(), "Error occured in updateListener, recording is aborted");
                e.printStackTrace();
            }
        }
     ...
     *
     */
    public WavRecorder(int audioSource, int sampleRate, int channelConfig, int audioFormat) {
        try {
            ...
            if (mBufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) {
                ...
                Log.w(WavRecorder.class.getName(), "Increasing buffer size to " + Integer.toString(mBufferSize));
            }
            ...
            if (e.getMessage() != null) {
                ...
                Log.e(WavRecorder.class.getName(), "Unknown error occured while initializing recording");
            }
            state = State.ERROR;
        }
    }
    ...
    public void setOutputFile(String argPath) {
        try {
            ...
            if (e.getMessage() != null) {
                ...
                Log.e(WavRecorder.class.getName(), "Unknown error occured while setting output path");
            }
            state = State.ERROR;
        }
    }
        ...
        try {
            if (state == State.INITIALIZING) {
                if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
                    // write file header
                    randomAccessWriter = new RandomAccessFile(filePath, "rws");
                    ...
                    Log.e(WavRecorder.class.getName(), "prepare() method called on uninitialized recorder");
                    state = State.ERROR;
                }
            } else {
                Log.e(WavRecorder.class.getName(), "prepare() method called on illegal state");
                release();
                state = State.ERROR;
            }
        } catch(Exception e) {
            if (e.getMessage() != null) {
                ...
                Log.e(WavRecorder.class.getName(), "Unknown error occured in prepare()");
            }
            state = State.ERROR;
        }
    }
        ...
        if (state == State.RECORDING) {
            ...
            if (state == State.READY){
                try {
                    ...
                    Log.e(WavRecorder.class.getName(), "I/O exception occured while closing output file");
                }
    ...
    public void start() {
        if (state == State.READY) {
            ...
            Log.e(WavRecorder.class.getName(), "start() called on illegal state");
            state = State.ERROR;
        }
    }
